...
let baseURL = `https://travellog-1.onrender.com/`;
...
function createCard(item) {
  ...
  let estimatedCostInINR = item.estimatedCost * 5;
  price.innerHTML = `<span>₹${estimatedCostInINR}</span> per person`; // Changed $ to ₹
      ...
      user
        .then((data) => {
          ...
          if (userObj.cart) {
            ...
            // destination.js ~line 215
           userObj.cart.push({
           city: item.name,
           numOfDays: 7, // Changed from 1
           numOfPeople: 1,
           tier: "Basic",
          basePrice: item.estimatedCost * 5,
          });
...
async function getUser(id) {
  let url = `https://travellog-1.onrender.com/users?id=${id}`;
  try {
    let res = await fetch(url);
    return res.json();
  } catch (e) {
...
async function postCart(id, userCart) {
  let url = `https://travellog-1.onrender.com/users/${id}`;
  try {
    let res = await fetch(url, {
      method: "PATCH",
      headers: {
